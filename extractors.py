...
class ResNet(nn.Module):
    def __init__(self,base_channel, block, layers=(3, 4, 23, 3)):
        self.inplanes = base_channel #64
        self.bc = base_channel
        ...
        self.layer1 = self._make_layer(block, self.bc, layers[0])
        self.layer2 = self._make_layer(block, self.bc*2, layers[1], stride=2)
        self.layer3 = self._make_layer(block, self.bc*4, layers[2], stride=1, dilation=2)
        self.layer4 = self._make_layer(block, self.bc*8, layers[3], stride=1, dilation=4)
        #
...
class DenseNet(nn.Module):
    def __init__(self, growth_rate=32, block_config=(6, 12, 24, 16),
        ...
        self.start_features = nn.Sequential(OrderedDict([
            ('conv0', nn.Conv2d(1, num_init_features, kernel_size=7, stride=2, padding=3, bias=False)), #3
...
class SqueezeNet(nn.Module):
    ...
    def __init__(self, pretrained=False):
        ...
        self.feat_1 = nn.Sequential(
            nn.Conv2d(1, 64, kernel_size=3, stride=2, padding=1), #squeezeNet
...
def resnet10(base_channel =64,pretrained=False):
    model = ResNet(base_channel,BasicBlock, [1, 1, 1, 1])
    if pretrained:
        load_weights_sequential(model, model_zoo.load_url(model_urls['resnet18']))
    return model

def resnet18(base_channel =64,pretrained=False):
    model = ResNet(base_channel,BasicBlock, [2, 2, 2, 2])
    if pretrained:
        load_weights_sequential(model, model_zoo.load_url(model_urls['resnet18']))
    return model


def resnet34(base_channel =64,pretrained=False):
    model = ResNet(base_channel,BasicBlock, [3, 4, 6, 3])
    if pretrained:
        load_weights_sequential(model, model_zoo.load_url(model_urls['resnet34']))
    return model


def resnet50(base_channel =64,pretrained=False):
    model = ResNet(base_channel,Bottleneck, [3, 4, 6, 3])
    if pretrained:
        load_weights_sequential(model, model_zoo.load_url(model_urls['resnet50']))
    return model


def resnet101(base_channel =64,pretrained=False):
    model = ResNet(base_channel,Bottleneck, [3, 4, 23, 3])
    if pretrained:
...
def resnet152(pretrained=False):
    model = ResNet(base_channel,Bottleneck, [3, 8, 36, 3])
    if pretrained:
...
if 0:
    if __name__=='__main__':
        base_channel=64
        model1 = ResNet(base_channel,BasicBlock,[2, 2, 2, 2])
        base_channel=64        
        model2 = ResNet(base_channel,Bottleneck, [3, 4, 6, 3])
        print("restnet10 channel 18 Total paramters:",count_param(model1))
        print("restnet10 channel 50  Total paramters:",count_param(model2))