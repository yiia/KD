...
class Encoder(nn.Module):
    def __init__(self, config, vis):
        ...
        for _ in range(config.num_layers_enc):
...
class Transformer(nn.Module):
    ...
    def forward(self, input_ids):
        embedding_output = self.embeddings(input_ids) #[B, N, D]
...
class VisionTransformer(nn.Module):
    def __init__(self, config, img_size=256,  zero_head=False, vis=True):
    ...
    def forward(self, x):
        if x.size()[1] == 1: 
            x = x.repeat(1,3,1,1)
        hidden_states,atten_weight = self.transformer(x) 
        return atten_weight
